        ...
        const jobOrdersTable = $('#job-order-table').DataTable({
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type) {
                        return `<div class="text-muted font-weight-bolder font-size-lg mb-0">${data.length > 0 && data.map(item => Number(item.pivot.produced)).reduce((acc, cur) => { return acc + cur }, 0)}</div>`;
                    },
                },
                {
                    data: 'action',
