...
<script>
    $(function() {
        const deliveryOrdersTable = $('#delivery-order-table').DataTable({
            processing: true,
            serverSide: true,
            ajax: '/datatables/delivery-orders',
            columns: [{
                    data: 'number',
                ...
                },
                {
                    ...
                    render: function(data, type) {
                        return `<div class="text-muted font-weight-bolder font-size-lg mb-0">${data.length > 0 && data.map(item => Number(item.pivot.amount)).reduce((acc, cur) => { return acc + cur }, 0)}</div>`;
                    },
                },
                {
                    data: 'action',
        ...
        $('#delivery-order-table').on('click', 'tr .btn-delete', function(e) {
            e.preventDefault();
            // alert('click');
            ...
            Swal.fire({
                ...
                showLoaderOnConfirm: true,
                preConfirm: () => {
                    return axios.delete('/delivery-order/' + id)
                        .then(function(response) {
                            console.log(response.data);
                        })
                        .catch(function(error) {
